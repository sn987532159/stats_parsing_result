...
KAO = pd.read_csv("AI_re/rh.aparc.pial.stats.csv")
AI = pd.read_csv("AI new/rh.aparc.pial.stats.csv")
...
KAO_AI.to_csv("AI_RESULT/AI_AI/AI_AI_rh.aparc.pial.stats.csv", index = False)

KAO_part = KAO_AI.loc[: , "bankssts-NumVert_KAO" : "insula-CurvInd_KAO"]
AI_part = KAO_AI.loc[: , "bankssts-NumVert_AI" : "insula-CurvInd_AI"]
...
rh_aparc_pial_stats = pd.DataFrame([Mean_KAO,sd_KAO,Mean_AI,sd_AI,Mean_Diff,Mean_Diff_Percentage,
...
rh_aparc_pial_stats.columns = ['Mean_AI_re','sd_AI_re','Mean_AI','sd_AI','Mean_Diff','Mean_Diff_%',
...
for i in KAO_part.columns:
    df_index.append(i[: -4])
rh_aparc_pial_stats.index = df_index
rh_aparc_pial_stats.index.name = "Structure"
    
rh_aparc_pial_stats.fillna("NaN", inplace=True)

rh_aparc_pial_stats.to_csv("AI_RESULT/COMPARISON/comparison_rh.aparc.pial.stats.csv")
