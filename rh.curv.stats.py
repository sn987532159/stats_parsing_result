...
KAO = pd.read_csv("AI_re/rh.curv.stats.csv")
AI = pd.read_csv("AI new/rh.curv.stats.csv")
...
KAO_AI.to_csv("AI_RESULT/AI_AI/AI_AI_rh.curv.stats.csv", index = False)
...
rh_curv_stats = pd.DataFrame([Mean_KAO,sd_KAO,Mean_AI,sd_AI,Mean_Diff,Mean_Diff_Percentage,
...
rh_curv_stats.columns = ['Mean_AI_re','sd_AI_re','Mean_AI','sd_AI','Mean_Diff','Mean_Diff_%',
...
for i in KAO_part.columns:
    df_index.append(i[: -4])
rh_curv_stats.index = df_index
rh_curv_stats.index.name = "Structure"
    
rh_curv_stats.fillna("NaN", inplace=True)

rh_curv_stats.to_csv("AI_RESULT/COMPARISON/comparison_rh.curv.stats.csv")
