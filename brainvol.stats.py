...
KAO = pd.read_csv("AI_re/brainvol.stats.csv")
AI = pd.read_csv("AI new/brainvol.stats.csv")
...
KAO_AI.to_csv("AI_RESULT/AI_AI/AI_AI_brainvol.stats.csv", index = False)

KAO_part = KAO_AI.loc[: , "Brain_Segmentation_Volum_mm3_KAO" : "Volume_of_ventricles_and_choroid_plexus_mm3_KAO"]
AI_part = KAO_AI.loc[: , "Brain_Segmentation_Volum_mm3_AI" : "Volume_of_ventricles_and_choroid_plexus_mm3_AI"]
...
brainvol_stats = pd.DataFrame([Mean_KAO,sd_KAO,Mean_AI,sd_AI,Mean_Diff,Mean_Diff_Percentage,
...
brainvol_stats.columns = ['Mean_AI_re','sd_AI_re','Mean_AI','sd_AI','Mean_Diff','Mean_Diff_%',
...
for i in KAO_part.columns:
    df_index.append(i[: -4])
brainvol_stats.index = df_index
brainvol_stats.index.name = "Structure"
    
brainvol_stats.fillna("NaN", inplace=True)

brainvol_stats.to_csv("AI_RESULT/COMPARISON/comparison_brainvol.stats.csv")
