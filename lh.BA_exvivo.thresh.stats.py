...
KAO = pd.read_csv("AI_re/lh.BA_exvivo.thresh.stats.csv")
AI = pd.read_csv("AI new/lh.BA_exvivo.thresh.stats.csv")
...
KAO_AI.to_csv("AI_RESULT/AI_AI/AI_AI_lh.BA_exvivo.thresh.stats.csv", index = False)
...
lh_BA_exvivo_thresh_stats = pd.DataFrame([Mean_KAO,sd_KAO,Mean_AI,sd_AI,Mean_Diff,Mean_Diff_Percentage,
...
lh_BA_exvivo_thresh_stats.columns = ['Mean_AI_re','sd_AI_re','Mean_AI','sd_AI','Mean_Diff','Mean_Diff_%',
...
for i in KAO_part.columns:
    df_index.append(i[: -4])
lh_BA_exvivo_thresh_stats.index = df_index
lh_BA_exvivo_thresh_stats.index.name = "Structure"
    
lh_BA_exvivo_thresh_stats.fillna("NaN", inplace=True)

lh_BA_exvivo_thresh_stats.to_csv("AI_RESULT/COMPARISON/comparison_lh.BA_exvivo.thresh.stats.csv")
