...
KAO = pd.read_csv("AI_re/lh.aparc.pial.stats.csv")
AI = pd.read_csv("AI new/lh.aparc.pial.stats.csv")
...
KAO_AI.to_csv("AI_RESULT/AI_AI/AI_AI_lh.aparc.pial.stats.csv", index = False)
...
lh_aparc_pial_stats = pd.DataFrame([Mean_KAO,sd_KAO,Mean_AI,sd_AI,Mean_Diff,Mean_Diff_Percentage,
...
lh_aparc_pial_stats.columns = ['Mean_AI_re','sd_AI_re','Mean_AI','sd_AI','Mean_Diff','Mean_Diff_%',
...
for i in KAO_part.columns:
    df_index.append(i[: -4])
lh_aparc_pial_stats.index = df_index
lh_aparc_pial_stats.index.name = "Structure"
    
lh_aparc_pial_stats.fillna("NaN", inplace=True)

lh_aparc_pial_stats.to_csv("AI_RESULT/COMPARISON/comparison_lh.aparc.pial.stats.csv")
