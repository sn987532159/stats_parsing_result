...
KAO = pd.read_csv("AI_re/lh.aparc.DKTatlas.stats.csv")
AI = pd.read_csv("AI new/lh.aparc.DKTatlas.stats.csv")
...
KAO_AI.to_csv("AI_RESULT/AI_AI/AI_AI_lh.aparc.DKTatlas.stats.csv", index = False)
...
lhaparc_DKTatlas_stats = pd.DataFrame([Mean_KAO,sd_KAO,Mean_AI,sd_AI,Mean_Diff,Mean_Diff_Percentage,
...
lhaparc_DKTatlas_stats.columns = ['Mean_AI_re','sd_AI_re','Mean_AI','sd_AI','Mean_Diff','Mean_Diff_%',
...
for i in KAO_part.columns:
    df_index.append(i[: -4])
lhaparc_DKTatlas_stats.index = df_index
lhaparc_DKTatlas_stats.index.name = "Structure"
    
lhaparc_DKTatlas_stats.fillna("NaN", inplace=True)

lhaparc_DKTatlas_stats.to_csv("AI_RESULT/COMPARISON/comparison_lh.aparc.DKTatlas.stats.csv")
